...
<body>
	...
	<div class="contents-wrap">
		<form action="${ctx }/dept/update.do" method="post">
		<input type="hidden" name="deptNo" value="${deptNo }">
			<h2 class="page-title">부서 추가 직원목록</h2>
			<div class="contents">
				<table border="1">
					<colgroup>
						<col width="90">
					...
					<tbody>
						<c:forEach items="${emps }" var="emp" varStatus="sts">
							
							<tr>
								<td><input type="radio" name="empNo" value="${emp.no }"></td>
								<td>${emp.no }</td>
								<td>${emp.name }</td>
								<td>${emp.deptNo }</td>
							</tr>
